var HOME='home';
var ENTITY_TOPMENU='topMenu';
var ENTITY_SUBMENU='subMenu';
var ENTITY_PICTURE='picture';
var ENTITY_PORTFOLIO='portfolio';
...
var add = function(entity) {
	...
	$("select[id$=subMenu-topMenu-list] > option").remove();
	$("select[id$=picture-subMenu-list] > option").remove();
	//checking the entity to populate the select box
	if (entity == ENTITY_SUBMENU) {
		// populating the topMenu by making an ajax call
		populateSelectBox('subMenu-topMenu-list', '/topMenu');
	}  else if (entity == ENTITY_PICTURE){
		// populating the subMenu by making an ajax call
		populateSelectBox('picture-subMenu-list','/subMenu');
...
var formValidate = function(entity){
	...
	switch(entity){
		case ENTITY_SUBMENU:
			var valueTopMenu = $('#subMenu-topMenu-list').val();
			if(valueTopMenu == "" || key == ""){
				showMessage('Sub Menu name and Top Menu selection are required.', entity);
				return;
			}
			break;
		case ENTITY_PICTURE:
			var valueSubMenu=$('#picture-subMenu-list').val();
			// var valueItem = $('#order-item-list').val();
			//if(valueCustomer == "" || valueItem==""){
			if(valueSubMenu == "" || key == ""){
				showMessage('Picture name and Sub Menu selection are required.', entity);
				return;
			}
			break;
		/*	
		case ENTITY_CUSTOMER:
			...
			break;
		*/
		// checking the first value which is usually name
		default :
			if(key==""){
				showMessage('The key value is required.', entity);
				return;
			}
			break;
	}
	...
	var successFn=function(resp){
		...
		if(data.length > 0){
			for (var i=0;i<data.length;i++){
				...
				{
				case ENTITY_TOPMENU:
					htm+='<td>'+data[i].name+'</td><td>'+data[i].includeExclude+'</td><td>'+data[i].description+'</td>';
					break;
				case ENTITY_SUBMENU:
					htm+='<td>'+data[i].name+'</td><td>'+data[i].includeExclude+'</td><td>'+data[i].topMenu+'</td><td>'+data[i].description+'</td>';
					break;
				case ENTITY_PICTURE:
					htm+='<td>'+data[i].name+'</td><td>'+data[i].includeExclude+'</td><td>'+data[i].subMenu+'</td><td>'+data[i].description+'</td>';
					break;
				/*
				case ENTITY_ORDER:
